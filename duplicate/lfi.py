...
class lfiResponse:
    ...
    def response(self, flow: http.HTTPFlow) -> None:
        ...
        if flow.request.method == "GET":
            ...
            self.process_parameters_lfi(flow, flow.request.query, method)
        elif flow.request.method == "POST":
            method = flow.request.method
            self.process_post_request_lfi(flow, method)

         #--- end first phase check the method request ---

    # ---- the porcess parameter called when is POST or GET
                
    def process_parameters_lfi(self, flow, params, method):
        ...
        if method == "POST":
            ...
            for key, content in all_processed_items.items():
                ...
                if re.search(url_pattern, str(content)):
                    self.handle_reflection_lfi(flow, key)
                ...
                try:
                    ...
                    if isinstance(decoded_json, dict) or isinstance(decoded_json, list):
                        ...
                        for key, content in all_processed_items.items():
                            url_pattern = r'(\w+:\/\/[^\s]+|\/[^?#\s]*(?=[?#\s]|$))'
                            if re.search(url_pattern, str(content)):
                                self.handle_reflection_lfi(flow, key, decoded_json, orginalgetparam)
                except:
                    url_pattern = r'(\w+:\/\/[^\s]+|\/[^?#\s]*(?=[?#\s]|$))'
    ...
    # ---- porcess post request lfi ----------------------------------------

    def process_post_request_lfi(self, flow, method):
        ...
        if "application/x-www-form-urlencoded" in content_type or "text/plain" in content_type:
            ...
            self.process_parameters_lfi(flow, params, method)
...
#---- ----------handle reflection function -----------------------------
    ...
    def handle_reflection_lfi(self, flow, param, dictoniry=None, orginalgetparam=None):
        with ThreadPoolExecutor(max_workers=100) as executor:
            # Logic to handle the reflection, e.g., logging, altering, and replaying
            for indicator in self.alteration_indicators_lfi:
                executor.submit(self.alter_and_replay_lfi, flow, param, indicator, dictoniry, orginalgetparam)


#---- ----------end of handle reflection function -----------------------------


#---- ---------- alter_and_replay_lfi function -----------------------------
    def alter_and_replay_lfi(self, original_flow, param, indicator, dictoniry, orginalgetparam):
        altered_value = indicator
        altered_flow = self.alter_request_lfi(original_flow, param, altered_value, dictoniry, orginalgetparam)
        if altered_flow:
            ctx.master.commands.call("replay.client", [altered_flow])
#---- ---------- end alter_and_replay_lfi function -----------------------------
...
#---- ---------- alter_request_lfi function -----------------------------

    def alter_request_lfi(self, original_flow, param, altered_value, dictoniry, orginalgetparam):
...
#---- ---------- end alter_request_lfi function -----------------------------

#---- ---------- check_altered_reflection_lfi function -----------------------------
    def check_altered_reflection_lfi(self, flow):
        ctx.log.info("check alterd reflection lfi")
