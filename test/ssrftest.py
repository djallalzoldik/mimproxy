...
class SSRF:
    def __init__(self):
        self.flow_dir_ssrf = "SSRF"
        os.makedirs(self.flow_dir_ssrf, exist_ok=True)
        # Payload templates for SSRF
        self.payload_template_ssrf = [
    "//www.example.com/%2f..",
        ...
        for watched_param in self.custom_srf_watched_params:
            for actual_param in params:
                if re.match(r"^{}$".format(watched_param), actual_param, re.IGNORECASE):
                    ...
                    self.alter_and_replay_ssrf(flow, actual_param, value)
    ...
    def alter_and_replay_ssrf(self, original_flow, param, original_value):
        ...
        for template in self.payload_template_ssrf:
    ...
    def save_flow_ssrf(self, flow):
        filename = os.path.join(self.flow_dir_ssrf, f"captured_request_{int(time.time())}.mitm")
        try:
            with open(filename, "wb") as file:
                fw = FlowWriter(file)
                fw.add(flow)
            ctx.log.info(f"SSRF detected {filename}")
        except OSError as e:
