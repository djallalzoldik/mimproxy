...
class LFI:
        ...
        for watched_param in self.watched_parameters_lfi:
            for actual_param in params:
                if re.match(r"^{}$".format(watched_param), actual_param, re.IGNORECASE):
                    ...
                    self.alter_and_replay_lfi(flow, actual_param, value)
    ...
    def alter_and_replay_lfi(self, original_flow, param, original_value):
        ...
        for template in self.payload_template_lfi:
            ...
            altered_flow = self.alter_request_lfi(original_flow, param, altered_value)
            if altered_flow:
    ...
    def save_flow_lfi(self, flow):
        filename = os.path.join(self.flow_dir_lfi, f"captured_request_{int(time.time())}.mitm")
        try:
