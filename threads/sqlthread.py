...
class SQLinjec:
    ...
    def custom_sql_inj_process_params(self, flow):
        ...
        for watched_param in self.custom_sql_inj_watched_params:
            for actual_param in params:
                # Define a regex pattern that matches 'id' with specific characters around it
                # Here, [A-Za-z0-9_-] specifies allowable characters before and after 'id'
                regex_pattern = rf"(^|[_\-A-Za-z0-9]){watched_param}([_\-A-Za-z0-9]|$)"
                if re.match(regex_pattern, actual_param, re.IGNORECASE):
                    value = params[actual_param][0] if isinstance(params[actual_param], list) else params[actual_param]
                    self.sqlinjection_alter_and_replay(flow, actual_param, value)
    ...
    def sqlinjection_alter_and_replay(self, original_flow, param, original_value):
        with ThreadPoolExecutor(max_workers=20) as executor:
            for altered_value in self.custom_sql_inj_payloads:
                ...
                executor.submit(self.threaded_replay, original_flow, param, altered_value)
    def threaded_replay(self, original_flow, param, altered_value):
        altered_flow = self.alter_request_sql(original_flow, param, altered_value)
        if altered_flow:
    ...
    def check_altered_reflection_sql(self, flow):
        ...
        pattern = r'(SQL error code: -?\d+|database error: .+|syntax error at .+|permission denied|access violation|query failed: .+|unable to connect to database|query syntax|duplicate entry .+|foreign key constraint fails|unexpected error in query)'
        response_text = flow.response.get_text()
        match = re.search(pattern, response_text, re.IGNORECASE)
        if self.response_time_lower_bound <= duration <= self.response_time_upper_bound or match:
            ctx.log.info(f" Time based SQL injection: {flow.request.url}")
