...
class LFI:
    ...
    def response_lfi(self, flow: http.HTTPFlow) -> None:
        ...
        if flow.request.method in ["GET", "POST"]:
            self.process_parameters_lfi(flow)

    def process_parameters_lfi(self, flow):
        params = {}  
        ...
        for watched_param in self.watched_parameters_lfi:
            if watched_param in params:
                value = params[watched_param][0] if isinstance(params[watched_param], list) else params[watched_param]
                self.alter_and_replay_lfi(flow, watched_param, value)

    def alter_and_replay_lfi(self, original_flow, param, original_value):
        host = original_flow.request.host
        for template in self.payload_template_lfi:
            altered_value_lfi = template.format(host=host)  
            altered_flow_lfi = self.alter_request_lfi(original_flow, param, altered_value_lfi)
            if altered_flow_lfi:
                ctx.master.commands.call("replay.client", [altered_flow_lfi])

    def alter_request_lfi(self, original_flow, param, altered_value_lfi):
        new_request = copy.deepcopy(original_flow.request)
        if original_flow.request.method == "GET":
            new_request.query[param] = altered_value_lfi
        else:
            content_type = new_request.headers.get("Content-Type", "")
            if "application/x-www-form-urlencoded" in content_type:
                params = urllib.parse.parse_qs(new_request.get_text())
                params[param] = [altered_value_lfi]
                ...
                try:
                    ...
                    if param in body and isinstance(body[param], str):
                        body[param] = altered_value_lfi
                        new_request.text = json.dumps(body)
                except json.JSONDecodeError:
                    pass
        new_request.headers[self.altered_header_lfi] = "true"
        altered_flow_lfi = http.HTTPFlow(original_flow.client_conn, original_flow.server_conn)
        altered_flow_lfi.request = new_request
        return altered_flow_lfi
    ...
    def save_flow_lfi(self, flow):
        ...
        try:
            with open(filename, "wb") as file:
                fw = FlowWriter(file)
                fw.add(flow)
            ctx.log.info(f"LFI detect {filename}")
        except OSError as e:
