...
class SSTI:
    ...
    def alter_and_replay_ssti(self, original_flow, param, original_value):
        for indicator in self.alteration_indicators_ssti:
            ...
            altered_flow = self.alter_request_ssti(original_flow, param, altered_value_ssti)
            if altered_flow:
    ...
    def check_altered_reflection_ssti(self, flow):
        # Replace '9112588498694640' with the expected reflected value for SSTI
        pattern = re.compile(r'9112588498694640', re.IGNORECASE)
    ...
    def save_flow_ssti(self, flow):
        filename = os.path.join(self.flow_dir_ssti, f"SSTI_{int(time.time())}.mitm")
        try:
